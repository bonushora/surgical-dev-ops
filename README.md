...
- **Fail-closed:** missing, invalid, expired, ambiguous, or unqualified evidence
...
agent and the closest target to the complete conversational development loop.
This recommendation grants no privileged authority and claims no universal
comparative superiority. See the
[bilingual AI provider selection guide](./docs/AI_PROVIDER_SELECTION_PT-BR.md).

ENGINEER mode adds an immutable proposal bound to the observed READ_FILE target
and the actual BEFORE SHA-256. The flow must stop at
...
This path uses the local Codex authentication already configured on the machine.
Codex remains cognitive-only and read-only; physical mutations continue through
the governed Surgical DevOps adapters.

The compatibility executable `surgical` is also provided. To run the suite:

On first launch in a human terminal, bilingual onboarding selects one of the
...
To use a profile only for the current invocation, without changing the saved preference:
...
To select the complete human-facing language only for the current invocation,
...
Orchestrator and the same BH-SEP/BH-SDP, R3, journal, Manifest CAS, and anti-replay contracts.
...
model, bilingual parity, stable RAW artifacts, and fail-closed behavior.
