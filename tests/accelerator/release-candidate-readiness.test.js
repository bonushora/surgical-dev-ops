...
test('release candidate identity is consistent across public and executable surfaces', () => {
  ...
  assert.match(read('accelerator/cli/surgical.js'), /const VERSION = '2\.6\.0-rc\.4';/);
  assert.match(read('README.md'), /Surgical DevOps v2\.6\.0-rc\.4/);
  assert.match(read('README_PT-BR.md'), /Surgical DevOps v2\.6\.0-rc\.4/);
  assert.doesNotMatch(
...
test('release candidate publishes equivalent English and Portuguese notes', () => {
  const english = read(
    'docs/releases/v2.6.0-rc.4.md'
  );
  const portuguese = read(
    'docs/releases/v2.6.0-rc.4_PT-BR.md'
  );

  assert.match(
    english,
    /\[v2\.6\.0-rc\.4_PT-BR\.md\]\(\.\/v2\.6\.0-rc\.4_PT-BR\.md\)/
  );
  assert.match(
    portuguese,
    /\[v2\.6\.0-rc\.4\.md\]\(\.\/v2\.6\.0-rc\.4\.md\)/
  );

  for (const document of [english, portuguese]) {
    assert.match(document, /ded6eaf/);
    assert.match(document, /33278958374/);
