    ...
    async runStreamed(prompt) {
      ...
      return {
        events: (async function* () {
          yield { type: 'thread.started', thread_id: 'scenario-3-thread' };
...
test(
  'Scenario 3 materializes an exact bounded proposal before mutation authority',
  { timeout: 240_000 },
  async (t) => {
    ...
    git(repository, ['config', 'user.name', 'Scenario 3 Regression']);
    ...
    while (
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for Scenario 3.\n${observed}`);
      }
      await new Promise((resolve) => setTimeout(resolve, 10));
    }

    input.write('conversation status\nexit\n');
    input.end();
    ...
    assert.match(observed, /An exact proposal is awaiting a decision/);
