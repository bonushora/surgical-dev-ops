...
function assertCurrentAdr038Manifest(manifest) {
  ...
  assert.deepEqual(target.runtimeCompletion, {
    ...
    qualifiedRuntimeCommit: QUALIFIED_RUNTIME_CANDIDATE,
    qualification: 'COMPLETE_GREEN',
    experience: 'GREEN',
    checkpointMeaning: 'R1-R7_INTERNAL_RUNTIME_CHECKPOINTS_NOT_OFFICIAL_ADR_MILESTONES'
  });
  assert.equal(target.packagePreparation.startingHead, '2714236e1aa7f9f6b971dd509889e46d819daea9');
  assert.equal(target.packagePreparation.physicalState, 'COMMITTED_CANDIDATE_FROZEN');
  assert.equal(target.packagePreparation.readiness, 'REVIEW_CANDIDATE_FROZEN');
  assert.equal(target.packagePreparation.reviewCandidateCommit, '2714236e1aa7f9f6b971dd509889e46d819daea9');
  assert.equal(target.packagePreparation.reviewPackageCommit, '1a9dd5aca16366c3a0f5525e8835e1c6b9f73ca9');
  assert.equal(target.packagePreparation.reviewShaFrozen, true);
