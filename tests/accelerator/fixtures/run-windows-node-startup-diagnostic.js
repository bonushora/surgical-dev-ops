...
try {
  const lines = typeof result.stdout === 'string'
    ...
    : [];
  const evidence = /^step=N[1-5] processCreated=(?:true|false) exitCode=(?:\d+|null) stage=[a-z-]+ stderrClass=(?:NATIVE_ABORT|EMPTY|OTHER|OUTPUT_LIMIT) fatalReason=[A-Za-z0-9_+-]+ subsystem=[A-Z_]+ nativeEventObserved=(?:true|false) exceptionCode=(?:0x[0-9A-F]+|NOT_OBSERVED) exceptionClass=[A-Z_]+ firstChance=(?:true|false|NOT_APPLICABLE) terminationClass=[A-Z_]+ terminationOrigin=[A-Z_]+ exitProcessCode=(?:\d+|NOT_OBSERVED) debugLoopStatus=[A-Z_]+ firstNativeFrame=[A-Za-z0-9_:<>~.+-]+ nativeFrames=[A-Za-z0-9_:<>~.+,-]+ markerPresent=true cleanup=(?:PASS|FAIL)$/;
  const transition = /^state=(?:CONTROL_N1|TEST_GOVERNED_STDIN|VERIFY_ORIGINAL_CONTROL|RUN_N2|RUN_N3|RUN_N4|RUN_N5|BLOCKED|COMPLETE) evidence=[A-Z0-9_]+ nextState=(?:CONTROL_N1|TEST_GOVERNED_STDIN|VERIFY_ORIGINAL_CONTROL|RUN_N2|RUN_N3|RUN_N4|RUN_N5|BLOCKED|COMPLETE) equivalentAttempts=\d+ breakerDecision=[A-Z0-9_]+$/;
  ...
  if (lines.length === 0 || invalidLine) {
    ...
    const transitionStates = new Set([
      ...
      'RUN_N2', 'RUN_N3', 'RUN_N4', 'RUN_N5', 'BLOCKED', 'COMPLETE'
    ]);
